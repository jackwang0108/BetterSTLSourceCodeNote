...
struct _Deque_iterator {
	...
	typedef _Tp **_Map_pointer;// 元素指针的指针
...
#	endif /* __SGI_STL_NO_ARROW_OPERATOR */
	...
	_Self &operator++() {
		...
		if (_M_cur == _M_last) {     // 到达缓冲区的尾端
			_M_set_node(_M_node + 1);// 切换至下一缓冲区的第一个元素
			_M_cur = _M_first;
		}
		return *this;
	}
...
protected:
	...
	_Tp **_M_allocate_map(size_t __n) { return _M_map_allocator.allocate(__n); }
	void _M_deallocate_map(_Tp **__p, size_t __n) { _M_map_allocator.deallocate(__p, __n); }
...
public:
	...
	_Deque_base(const allocator_type &__a, size_t __num_elements)
	    : _Base(__a), _M_start(), _M_finish() { _M_initialize_map(__num_elements); }
...
#	endif /* __STL_THROW_RANGE_ERRORS */
	...
	reference front() { return *_M_start; }// 调用operator*
	reference back() {
		iterator __tmp = _M_finish;
		--__tmp;
		return *__tmp;
		// 不能改为return *(finish-1), 是因为没有定义迭代器和int的运算子operator-
	}
...
public:// Constructor, destructor.
	...
	deque(size_type __n, const value_type &__value,
	      const allocator_type &__a = allocator_type()) : _Base(__a, __n) { _M_fill_initialize(__value); }
...
#	else /* __STL_MEMBER_TEMPLATES */
	...
	deque(const value_type *__first, const value_type *__last,
	    ...
	    : _Base(__a, __last - __first) { uninitialized_copy(__first, __last, _M_start); }
...
private:// helper functions for assign()
	template<class _Integer>
	void _M_assign_dispatch(_Integer __n, _Integer __val, __true_type) { _M_fill_assign((size_type) __n, (_Tp) __val); }
...
public:// Insert
	...
	iterator insert(iterator __position) { return insert(__position, value_type()); }

	void insert(iterator __pos, size_type __n, const value_type &__x) { _M_fill_insert(__pos, __n, __x); }
...
protected:// Allocation of _M_map and nodes
	...
	void _M_reserve_map_at_back(size_type __nodes_to_add = 1) {
		// 如果map尾端的节点备用空间不足，配置更大的空间，拷贝原来的，释放原来的
		if (__nodes_to_add + 1 > _M_map_size - (_M_finish._M_node - _M_map))
			_M_reallocate_map(__nodes_to_add, false);
	}

	void _M_reserve_map_at_front(size_type __nodes_to_add = 1) {
		// 如果map前端的节点备用空间不足，配置更大的空间，拷贝原来的，释放原来的
...
// 缓冲区只剩一个备用元素空间，配置新节点（缓冲区）
// Called only if _M_finish._M_cur == _M_finish._M_last - 1.
template<class _Tp, class _Alloc>
void deque<_Tp, _Alloc>::_M_push_back_aux() {
	_M_reserve_map_at_back();
	*(_M_finish._M_node + 1) = _M_allocate_node();// 配置新节点
	__STL_TRY {
		construct(_M_finish._M_cur);
		_M_finish._M_set_node(_M_finish._M_node + 1);
		_M_finish._M_cur = _M_finish._M_first;
...
void deque<_Tp, _Alloc>::_M_push_front_aux() {
	_M_reserve_map_at_front();
	*(_M_start._M_node - 1) = _M_allocate_node();// 配置新缓冲区
	__STL_TRY {
		_M_start._M_set_node(_M_start._M_node - 1);
		_M_start._M_cur = _M_start._M_last - 1;
