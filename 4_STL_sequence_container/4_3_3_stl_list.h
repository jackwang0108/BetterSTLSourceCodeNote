...
protected:
	_List_node<_Tp> *_M_get_node() { return _Node_allocator.allocate(1); }
	void _M_put_node(_List_node<_Tp> *__p) { _Node_allocator.deallocate(__p, 1); }
...
public:
	...
	_List_base(const allocator_type &__a) : _Base(__a) {
		_M_node = _M_get_node();   // 配置一个节点空间，令node指向它
		_M_node->_M_next = _M_node;// node头尾都指向自己
