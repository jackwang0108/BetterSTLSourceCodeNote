...
enum { __stl_num_primes = 28 };// 28个质数
...
public:
	...
	typedef _Val value_type;
	typedef _HashFcn hasher;    // 为template类型参数重新定义名称
	typedef _EqualKey key_equal;// 为template类型参数重新定义名称
	...
	// 以下为函数对象，stl_hash_fun.h定义有标准类型的hasher
...
private:
	...
	vector<_Node *, _Alloc> _M_buckets;// 以vector实现
...
public:
	...
	pair<iterator, bool> insert_unique(const value_type &__obj) {
		resize(_M_num_elements + 1);// 判断是否需要重建表格，需要则扩充
...
#	endif /*__STL_MEMBER_TEMPLATES */
	...
	iterator find(const key_type &__key) {
		size_type __n = _M_bkt_num_key(__key);// 寻找在哪个bucket内
		_Node *__first;
		// 从头开始比对键值，成功则跳出
...
private:
	...
	void _M_initialize_buckets(size_type __n) {
		// 假设n为50，则计算出质数53，保留53个元素空间，其他全部填0
	...
	size_type _M_bkt_num_key(const key_type &__key, size_t __n) const {
		return _M_hash(__key) % __n;// SGI内置hash()
	}
...
hashtable<_Val, _Key, _HF, _Ex, _Eq, _All>::insert_equal_noresize(const value_type &__obj) {
	// 决定obj 应位于 #n bucket，first指向链表头部
	const size_type __n = _M_bkt_num(__obj);
	_Node *__first = _M_buckets[__n];

	// 如果buckets[n]已被占用，first不为0，进入循环
	for (_Node *__cur = __first; __cur; __cur = __cur->_M_next)
		if (_M_equals(_M_get_key(__cur->_M_val), _M_get_key(__obj))) {
			// 发现键值重复，直接插入返回
			_Node *__tmp = _M_new_node(__obj);// 产生新节点
			// 插入位置是第一重复键值的后面？那么以后重复的键值顺序是15432？
			__tmp->_M_next = __cur->_M_next;// 插入目前位置
			__cur->_M_next = __tmp;
...
void hashtable<_Val, _Key, _HF, _Ex, _Eq, _All>::resize(size_type __num_elements_hint) {
	// 表格是否重建的条件是：
	//     加入新增元素后的元素个数与bucket vector大小比较
	//     前者大于后者则重建表格
	// 每个bucket（list）的最大容量和buckets vector大小相同
...
void hashtable<_Val, _Key, _HF, _Ex, _Eq, _All>::clear() {
	...
	for (size_type __i = 0; __i < _M_buckets.size(); ++__i) {
		...
		// 将bucket list中每一个元素删除
		while (__cur != 0) {
			_Node *__next = __cur->_M_next;
			_M_delete_node(__cur);
			__cur = __next;
		}
		_M_buckets[__i] = 0;// null指针
	}
	_M_num_elements = 0;// 清零
	                    // 注意buckets vector未释放空间，保有原来大小
}
...
void hashtable<_Val, _Key, _HF, _Ex, _Eq, _All>::_M_copy_from(const hashtable &__ht) {
	// 调用clear，使所有元素为0
	_M_buckets.clear();
	// 设置保留空间，尾端开始设置null指针，即从头开始设置null指针
	...
	__STL_TRY {
		...
		_M_num_elements = __ht._M_num_elements;// 设置节点个数
	}
	__STL_UNWIND(clear());
}
