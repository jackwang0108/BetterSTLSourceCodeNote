...
// 由于InputIterators，不保证赋值行为
// 如果要改变元素内容，应该使用transform
// for_each.  Apply a function to every element of a range.
template<class _InputIter, class _Function>
_Function for_each(_InputIter __first, _InputIter __last, _Function __f) {
	__STL_REQUIRES(_InputIter, _InputIterator);
	for (; __first != __last; ++__first)
		__f(*__first);// 调用f的函数调用操作符
	return __f;
}
...
_ForwardIter adjacent_find(_ForwardIter __first, _ForwardIter __last) {
	...
	while (++__next != __last) {
		if (*__first == *__next)// 如果找到相邻元素值相同，则结束
			return __first;
		__first = __next;
	}
	return __last;
...
void count_if(_InputIter __first, _InputIter __last, _Predicate __pred,
	...
	__STL_UNARY_FUNCTION_CHECK(_Predicate, bool,
	                           typename iterator_traits<_InputIter>::value_type);
	for (; __first != __last; ++__first)
		if (__pred(*__first))// 代入一元谓词运算结果为true则加1
			++__n;
}
...
_OutputIter replace_copy(_InputIter __first, _InputIter __last,
	...
	__STL_REQUIRES_BINARY_OP(_OP_EQUAL, bool,
	                         typename iterator_traits<_InputIter>::value_type, _Tp);
	for (; __first != __last; ++__first, ++__result)
		// 如果旧序列元素等于old_value，就放new_value到新序列中
		// 否则拷贝一份放进新序列
...
void __reverse(_RandomAccessIter __first, _RandomAccessIter __last,
               random_access_iterator_tag) {
	while (__first < __last)// 只有random iterator可以进行这种判断
		iter_swap(__first++, --__last);
}

// dispatch function，依赖参数推导机制
template<class _BidirectionalIter>
...
// 旋转数组 forward iterator版本
// 与swap_ranges区别：rotate可以交换不同长度区间
...
void random_shuffle(_RandomAccessIter __first, _RandomAccessIter __last,
                    _RandomNumberGenerator &__rand) {// 传递引用
	__STL_REQUIRES(_RandomAccessIter, _Mutable_RandomAccessIterator);
	if (__first == __last) return;
	for (_RandomAccessIter __i = __first + 1; __i != __last; ++__i)
...
// pred为true的元素放在前端，false放后端，不稳定算法
// 快排实现的部分
template<class _ForwardIter, class _Predicate>
...
// 分割函数
// 分为两段，pivot所在位置不确定是最终位置
template<class _RandomAccessIter, class _Tp>
_RandomAccessIter __unguarded_partition(_RandomAccessIter __first,
                                        _RandomAccessIter __last,
                                        _Tp __pivot) {
	while (true) {
		while (*__first < __pivot)// first找到>=pivot的元素就停下
			++__first;
		--__last;                // 调整
		while (__pivot < *__last)// last找到<=pivot的元素就停下
			--__last;
		if (!(__first < __last))// 结束条件
			return __first;
...
inline void __linear_insert(_RandomAccessIter __first,
	...
	_Tp __val = *__last;
	if (__val < *__first) {// 尾部比头部还小
		// 整个区间向右移动一个位置后，令头元素等于原来尾元素值
...
// 部分排序，使用heap算法，只接受randomAccessIterator
template<class _RandomAccessIter, class _Tp>
...
_ForwardIter __lower_bound(_ForwardIter __first, _ForwardIter __last,
	...
	_Distance __len = 0;
	distance(__first, __last, __len);// 取区间长度len
	_Distance __half;
	_ForwardIter __middle;
...
__equal_range(_ForwardIter __first, _ForwardIter __last, const _Tp &__val,
	...
	while (__len > 0) {     // 区间未遍历完，二分查找
		__half = __len >> 1;// 找出中间位置
		__middle = __first;
		advance(__middle, __half);
		if (*__middle < __val) {
			__first = __middle;
			...
			__len = __half;
		else {// 中央元素等于指定值，分别找lower_bound，upper_bound
...
inline void __inplace_merge_aux(_BidirectionalIter __first,
	...
	_Distance __len1 = 0;
	distance(__first, __middle, __len1);// len1为序列一长度
	_Distance __len2 = 0;
	distance(__middle, __last, __len2);// len2为序列二长度

	// 缓冲区
	_Temporary_buffer<_BidirectionalIter, _Tp> __buf(__first, __last);
	if (__buf.begin() == 0)/// 配置失败，使用没有缓冲区版本
...
bool includes(_InputIter1 __first1, _InputIter1 __last1,
	...
	__STL_REQUIRES(typename iterator_traits<_InputIter1>::value_type,
	               _LessThanComparable);
	while (__first1 != __last1 && __first2 != __last2)
		if (*__first2 < *__first1)// 序列二元素小于一元素，返回不成立
			return false;
...
_OutputIter set_union(_InputIter1 __first1, _InputIter1 __last1,
	...
	// 剩下的直接拷贝
	// [__first1, __last1]和[__first2, __last2]至少一个为空白区间
	return copy(__first2, __last2, copy(__first1, __last1, __result));
}
...
bool next_permutation(_BidirectionalIter __first, _BidirectionalIter __last) {
	...
	for (;;) {
		...
		--__i;
		if (*__i < *__ii) {                 // 前者小于后者
			_BidirectionalIter __j = __last;// j指向尾端
			while (!(*__i < *--__j))        // 往前找直到遇到比i大的元素
			{}
			iter_swap(__i, __j);  // 交换i， j
			reverse(__ii, __last);// 将ii之后的元素全部逆向重排
			return true;
		}
		if (__i == __first) {        // 到了最前面
...
bool prev_permutation(_BidirectionalIter __first, _BidirectionalIter __last) {
	...
	for (;;) {
		...
		--__i;
		if (*__ii < *__i) {                 // 前者大于后者
			_BidirectionalIter __j = __last;// 从后往前找比i小的元素
			while (!(*--__j < *__i)) {}
			iter_swap(__i, __j);  // 交换i，j
			reverse(__ii, __last);// ii之后元素逆向重排
			return true;
		}
		if (__i == __first) {        // 到了最前面
			reverse(__first, __last);// 全部逆向
			return false;
		}
	}
}
...
_ForwardIter1 __find_end(_ForwardIter1 __first1, _ForwardIter1 __last1,
	...
	if (__first2 == __last2)// 查找为空，返回last1
		return __last1;
	else {
		...
		while (1) {
			...
			if (__new_result == __last1)
				...
				__result = __new_result;// 调整准备下次查找
