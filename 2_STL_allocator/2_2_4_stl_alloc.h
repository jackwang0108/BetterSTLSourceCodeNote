...
#			define __NODE_ALLOCATOR_LOCK \
				if (threads && __us_rsthread_malloc) { _S_node_allocator_lock._M_acquire_lock(); }
#			define __NODE_ALLOCATOR_UNLOCK \
				if (threads && __us_rsthread_malloc) { _S_node_allocator_lock._M_release_lock(); }
...
#		endif
	// 向上取整至8的倍数（取反加一的逆操作）
	static size_t
	_S_round_up(size_t __bytes) { return (((__bytes) + (size_t) _ALIGN - 1) & ~((size_t) _ALIGN - 1)); }
...
public:
	...
	void deallocate(pointer __p, size_type __n) { _Alloc::deallocate(__p, __n * sizeof(_Tp)); }
...
struct __allocator {
	...
	// __p is not permitted to be a null pointer.
	void deallocate(pointer __p, size_type __n) { __underlying_alloc.deallocate(__p, __n * sizeof(_Tp)); }
